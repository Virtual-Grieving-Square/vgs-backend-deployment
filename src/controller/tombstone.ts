...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    const fileName = `uploads/image/tombstone/${Date.now()}-${removeSpaces(fileOrgnName)}`;
    ...
    res.status(200).json({ tombstone: tombstone, message: "Tombstone created successfully" });
