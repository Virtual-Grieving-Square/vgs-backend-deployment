import { Request, Response } from "express";
import { UserModel } from "../model/user";
import { handleAuthentication } from "../util/socketAuthentication";

export const Connect = async (req: Request, res: Response) => {
  try {
    const data = req.body;
    console.log("connection started");
    console.log("connection header", req.headers);
    console.log("connection query", req.query);
    console.log("connection body", req.body);

    console.log("connection started");

    res.status(200).json({ msg: "connection recived" });
  } catch (error) {
    console.error("connection error", error);
    res.status(500).json({ message: "Internal server error" });
  }
};

export const Disconnect = async (req: Request, res: Response) => {
  try {
    console.log("Disconnected");

    const data = req.body;

    res.status(200).json({ msg: "Webhook disconnected" });
  } catch (error) {
    console.error("Disconnection error", error);
    res.status(500).json({ message: "Internal server error" });
  }
};

export const Defualt = async (req: Request, res: Response) => {
  try {
    const data = req.body;

    switch (data.action) {
      case "client_like_update":
        res.status(200).json({ event: "server_update_like", data: data.data });
        break;
      case "client_new_post":
        res.status(200).json({ event: "server_new_post" });
        break;
      case "client_new_memorial":
        res.status(200).json({ event: "server_new_memorial" });
        break;
      case "client_comment_update":
        res.status(200).json({ event: "server_comment_update" });
        break;
      case "client_new_hero":
        res.status(200).json({ event: "server_new_hero" });
        break;
      case "client-stripe-account-setup-complete":
        res.status(200).json({ event: "server-stripe-account-setup-complete" });
        break;
      default:
        res.status(400).json({ error: "Unknown action" });
        break;
    }
  } catch (error) {
    console.error("Disconnection error", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
